...
function getFieldSchema(field) {
	const arrayMatch = typeof field.type === "string" ? /^(string|number)\[\]$/.exec(field.type) : null;
	const schema = arrayMatch ? {
		type: "array",
		items: { type: arrayMatch[1] }
	} : {
...
function schemaAcceptsUndefined(zodType) {
	if (zodType instanceof z.ZodOptional || zodType instanceof z.ZodDefault || zodType instanceof z.ZodPrefault || zodType instanceof z.ZodCatch || zodType instanceof z.ZodAny || zodType instanceof z.ZodUnknown || zodType instanceof z.ZodUndefined || zodType instanceof z.ZodVoid) return true;
...
function toOpenApiSchema(zodType) {
	...
	if (zodType instanceof z.ZodAny || zodType instanceof z.ZodUnknown) return withDescription({}, zodType);
