...
import { addOAuthServerContext, getOAuthState } from "../../api/state/oauth.mjs";
...
*/
async function resolveAnonymousSession(ctx) {
	const cookieSession = await getSessionFromCtx(ctx, { disableRefresh: true });
	if (cookieSession?.user.isAnonymous) return {
	...
	const anonymousUserId = (await getOAuthState())?.serverContext?.anonymousUserId;
	if (typeof anonymousUserId !== "string") return null;
	const user = await ctx.context.internalAdapter.findUserById(anonymousUserId);
	if (!user?.isAnonymous) return null;
	const [anonymousSession] = await ctx.context.internalAdapter.listSessions(user.id, { onlyActiveSessions: true });
		...
		hooks: {
			before: [{
				...
				handler: createAuthMiddleware(async (ctx) => {
					...
					await addOAuthServerContext({ anonymousUserId: session.user.id });
				})
			}],
			after: [{
				matcher(ctx) {
